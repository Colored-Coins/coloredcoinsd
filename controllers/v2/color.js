module.exports = (function () {
  ...
  color.registerRoutes = function registerRoutes (app, path, swagger, allowAddEndpoint) {
    ...
    var getAssetMetadata = {
      ...
      'action': function (req, res) {
        api.getAssetMetadata(req.params.assetId, req.params.utxo).then(
    ...
    var getAddressInfo = {
      ...
      'action': function (req, res) {
        api.getAddressInfo(req.params.address).then(function (data) {
  ...
  function tryBroadcastAsset (req, res) {
    ...
    api.broadcastTx(req.body.txHex).then(function (txid) {
  ...
  function tryIssueAsset (req, res) {
    ...
    validateInput(req.body).then(checkParameters).then(api.uploadMetadata).then(api.createIssueTransaction).then(function (data) {
  ...
  function checkParameters (input) {
    ...
    if (input && (input.to || input.transfer)) {
      ...
      if (transferArr.some(function (transfer) {
        ...
        return false
        })) { deferred.reject(new Error('missing parameter address or pubKeys in transfer object')) } else { deferred.resolve(input) }
    } else {
  ...
  function trySendAsset (req, res) {
    ...
    try {
      ...
      validateInput(req.body, null, ['from', 'sendutxo']).then(checkParameters).then(api.uploadMetadata).then(api.createSendAssetTansaction).then(function (data) {
        api.seedMetadata(data.metadata.sha1)
  ...
  function generateLocalMetadataPath (AssetDefinition) {
    var path
    if (config.useS3) {
      path = 'https://s3.amazonaws.com/coloredcoin-assets/' + AssetDefinition.short_name
      return path
    } else {
      path = config.machineurl + '/metadata/' + AssetDefinition.short_name
      return path
    }
  }
